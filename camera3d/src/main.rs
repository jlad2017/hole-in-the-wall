...
impl Camera for LOLCamera {
    fn new() -> Self {
        Self {
            pitch: 1.0,
            player_pos: Pos3::new(0.0, 0.0, 0.0),
            camera_pos: Pos3::new(0.0, 0.0, 0.0),
            follow_player: false,
        }
    }
    ...
    fn update(&mut self, events: &engine3d::events::Events, player: &Player) {
        ...
        if events.key_pressed(KeyCode::Y) {
        ...
        if self.follow_player {
            ...
            let mouse_pos = events.mouse_pos();
            self.camera_pos = Pos3::new(mouse_pos.0, self.camera_pos.y, mouse_pos.1);
        }
    }

    fn update_camera(&self, c: &mut engine3d::camera::Camera) {
        c.eye = self.camera_pos + Vec3::new(0.0, 25.0, 0.0);
        c.target = c.eye
...
fn main() {
    ...
    run::<GameData, Game<LOLCamera>>(window, std::path::Path::new("content"));
}
