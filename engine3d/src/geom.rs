...
impl Collide<Plane> for Box {
    fn touching(&self, p: &Plane) -> bool {
        ...
        if (v.dot(p.n) - p.d).signum() != dist.signum() { return true; }
        let v = self.c + self.axes.x * self.half_sizes.x
                       + self.axes.y * self.half_sizes.y
                       - self.axes.z * self.half_sizes.z;
        if (v.dot(p.n) - p.d).signum() != dist.signum() { return true; }
...
impl Collide<Box> for Box {
    fn touching(&self, b: &Box) -> bool {
        ...
        trans = Vec3::new(trans.dot(self.axes[0]), trans.dot(self.axes[1]), trans.dot(self.axes[2]));
        ...
        for i in 0..3 {
            ...
            let rb = b.half_sizes[0] * absrot[0][i] + b.half_sizes[1] * absrot[1][i] + b.half_sizes[2] * absrot[2][i];
        ...
        for i in 0..3 {
            let ra = self.half_sizes[0] * absrot[i][0] + self.half_sizes[1] * absrot[i][1] + self.half_sizes[2] * absrot[i][2];
